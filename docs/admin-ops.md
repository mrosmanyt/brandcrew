...
| **Customers** | Recent users (or email search) → plan, usage, memberships, jobs. **Assistant Pro do** / **Assistant Pro hatao** upsert `AssistantSubscription` (default +30 days, optional expiry) without changing `Workspace.plan`. Audit: `assign_assistant_pro` / `revoke_assistant_pro`. Suspend disables jobs. Accounts are not hard-deleted. |
