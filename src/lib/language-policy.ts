 ...
 * English-only / hospitality-only refusals.
 */
...
- Prefer your niche (hospitality, outreach, Brand Kit, desk jobs) when the request is about that work.
- Do not refuse basic helpful answers or language switching. For general Q&A on the desk, answer briefly, then offer to help with brand or desk work.`;

export const AGENT_HELPFULNESS_SUFFIX =
  " Reply in the user's language (including Urdu/Roman Urdu). Never claim English-only. Prefer this role's niche, but do not refuse basic helpful answers — answer briefly, then offer brand or desk work.";

export function withLanguagePolicy(system: string): string {
  const body = system.trim();
  if (!body) return LANGUAGE_AND_SCOPE_RULE;
  if (body.includes("Never refuse to speak a language")) return body;
...
export function withAgentHelpfulness(instructions: string): string {
  ...
  if (body.includes("Never claim English-only")) return body;
