...
import { demoAppHtml, demoWebsiteHtml } from "@/lib/demo";
...
export type BuilderKind = "website" | "app";
...
export async function generateBuilderArtifact(input: {
  ...
  const result = await llm.complete({
    ...
    messages: [
      {
        role: "system",
        content: `You are ${name} on CINEM Pro, a ${input.kind === "app" ? "app" : "website"} builder.
