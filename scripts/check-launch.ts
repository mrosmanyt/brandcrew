...
assert.match(makeIcon, /paintNightTileA/);
...
async function main() {
  ...
  assert.equal(SITE_ORIGIN, "https://app.cinem.tech");
  ...
  assert.equal(siteOrigin(), "https://preview.example");
  delete process.env.NEXT_PUBLIC_APP_URL;
  const savedVercelEnv = process.env.VERCEL_ENV;
  const savedVercelUrl = process.env.VERCEL_URL;
  process.env.VERCEL_ENV = "preview";
  process.env.VERCEL_URL = "brandcrew-git-foo.vercel.app";
  assert.equal(siteOrigin(), "https://brandcrew-git-foo.vercel.app");
  process.env.NEXT_PUBLIC_APP_URL = "https://preview.example";
  assert.equal(siteOrigin(), "https://preview.example");
  delete process.env.NEXT_PUBLIC_APP_URL;
  ...
  assert.match(layout, /CINEM_APP_ICON_SRC|\/icon\.svg/);
  assert.match(layout, /CINEM_FAVICON_ICO_SRC|\/favicon\.ico/);
  assert.match(layout, /CINEM_FAVICON_VERSION/);
